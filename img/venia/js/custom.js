...
.then((response) => {
  ...
  .then(data => {
    data.forEach(user => {
       ...
       const container =`<div><img src="${user.image}"/><h2>${user.title}</h2><p> ${user.price}</p><span class="icon-container"><i class="fa-regular fa-heart"></i><span> ${user.rating.rate}</span></span></div>`;
